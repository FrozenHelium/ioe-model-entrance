...
int __stdcall WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmdLine, int cmdShow)
{
    WNDCLASSEX wcex = { sizeof(WNDCLASSEX), CS_HREDRAW | CS_VREDRAW, FrobiWndProc, 0, 0, hInst, NULL,
        LoadCursor(NULL, IDC_ARROW), (HBRUSH)(COLOR_WINDOWFRAME), NULL, "__frobi_question_editor_window__", NULL };
    if (!RegisterClassEx(&wcex))
    {
        MessageBox(NULL, "failed to register window class", "Error!", MB_OK | MB_ICONERROR);
        return 0;
    }
    HWND hWnd = CreateWindowEx(0, "__frobi_question_editor_window__", "Question Editor for IoE Model Entrance", WS_OVERLAPPEDWINDOW^WS_SIZEBOX^WS_MAXIMIZEBOX,
